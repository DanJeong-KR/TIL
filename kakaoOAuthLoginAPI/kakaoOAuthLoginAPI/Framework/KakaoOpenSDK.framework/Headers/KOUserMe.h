 ...
 *             - 최초 로그인했을 때 발급<br>
 *             - 자동연결을 off한 상태로 signup 호출에 성공했을 때 발급<br>
 *             [사용자 아이디 고정]이 활성화되지 않은 앱이 연결해제(unlink)를 실행한 후 다시 연결하면 새로운 값으로 재발급됩니다. [사용자 아이디 고정]이 활성화된 앱은 동일 앱, 동일 카카오계정에 대하여 항상 같은 아이디가 부여됩니다.
 */
